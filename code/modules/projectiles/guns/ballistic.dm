	...
	if(istype(casing)) //there's a chambered round
		if(QDELING(casing))
			...
			if(ismob(shooter))
				...
				casing.AddComponent(/datum/component/movable_physics, horizontal_velocity = rand(450, 550) / 100, vertical_velocity = rand(400, 450) / 100, horizontal_friction = rand(20, 24) / 100, z_floor = 0, angle = angle_of_movement)
